...
function find_closest(target_lat, target_lng) {
	var distance;
	var best_distance = 0;
	var best_index = -1;
	
	for(var i=0;i<c1.length;i++) {
		distance = parseFloat(distVincenty(Geo.parseDMS(target_lat),Geo.parseDMS(target_lng),Geo.parseDMS(c1[i]),Geo.parseDMS(c2[i])));
	...
	if(best_index>-1) {
		return [c1[best_index],c2[best_index]];
	} else {
		return false;
	}
}
...
if (typeof register == 'function') { register("cities.js"); }